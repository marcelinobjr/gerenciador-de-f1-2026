...
export interface TrackEngineeringAndStrategySectionProps {
  ...
  practiceFeedbacks?: StintFeedbackRecord[] | null
  driverTireInventories?: Record<string, any> | null
  team: TeamModel | null
...
> = ({
  ...
  const informedPackage: PreparationInformedPackage = React.useMemo(() => {
    return buildPreparationInformedPackage({
      sessionKey: sessKey,
      currentSetup,
      gpInfo,
      weather: typeof weather === 'string' ? (weather as any) : 'seco',
      setupKnowledge,
      tyreKnowledge,
  ...
  const handleApplyRecommendedSetup = (adjustments: Partial<SessionSetupModel>) => {
    Object.entries(adjustments).forEach(([key, val]) => {
      updateCurrentSetup(key as keyof SessionSetupModel, val)
    })
  }

  return (
    <div className="space-y-6">
      {/* Bloco Compacto: RECOMENDAÇÃO DA ENGENHARIA (Etapa 4D.1) */}
      <EngineeringRecommendationCard
        informedPackage={informedPackage}
        onApplyRecommendedSetup={handleApplyRecommendedSetup}
        disabledApply={isDone || isSimulatingSession}
      />
        ...
        <Card className="relative z-10 bg-[#090D15]/80 backdrop-blur-md border border-[#1A2333] lg:col-span-2 shadow-xl">
          <CardHeader className="pb-3 border-b border-[#1A2333]">
            ...
            <CardDescription className="text-xs text-[#8B95A7] font-mono mt-0.5">
              Ajuste fino de aerodinâmica ativa, suspensão mecânica e gestão do trem de força
              híbrido para {gpInfo.circuit}.
            </CardDescription>
          </CardHeader>
          ...
          <CardContent className="space-y-6">
            ...
            <div className="space-y-2">
              <div className="flex justify-between items-center text-xs font-mono">
                <span className="text-[#F5F7FA] font-bold flex items-center gap-1.5">
                  <Zap className="w-4 h-4 text-amber-400" /> Balanço de Potência 50/50 (Elétrico
                  MGU-K vs Combustão V6):
                </span>
